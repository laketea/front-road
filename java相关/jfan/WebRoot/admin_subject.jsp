...
<table width="900" border="0" cellpadding="0" cellspacing="0" class="CContent"  >
	...
	%>	 
		 <table width="600" cellpadding="2" cellspacing="1" class="TablePanel" >
			<tr>
	          <td  class="TDHead" colspan="3"   >一级分类</td>
			</tr>
			<tr>
			  <td colspan="3" >
			    <form action="<%=request.getContextPath()%>/userAction.do" method="post" >添加一级分类
			    <input type="hidden" name="method" value="addfirstsubject"><input type="text"  name="username"  />序列号<input type="text"  name="point"  />&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="添加"  />
		      </form></td>
		    </tr>
			<tr>
			  <td width="172" class="TDHead">分类名称</td>
			  <td width="207" class="TDHead">排序</td>
			  <td width="207" class="TDHead">操作</td>
			</tr>
			 
			<%for(int i=0;i<results.size();i++){
				Subject subject  =(Subject)results.get(i);
			%>
			<form action="userAction.do" method="post" > <input type="hidden" name="method" value="modsubject" > <input type="hidden" name="id" value="<%=subject.getId().toString()%>" >
 			<tr><td><input type="text" name="username" value="<%=subject.getFirstname()%>" ></td>
			<td><input type="text" name="points" value="<%=subject.getPoint()%>" > </td>
			<td><input type="submit" value="修改" >&nbsp;&nbsp;<a href="#" onClick="confirmdelete(<%=subject.getId() %>)" >删除</a></td>
			</tr>
			</form>
			<%} %>
		</table>
